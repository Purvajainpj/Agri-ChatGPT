...
from gbb_ai.pdf_utils_docintel import extract_text_from_pdf_bytes
from gbb_ai.pdf_utils_docintel import extract_text_from_docx_bytes
# load logging
...
class SharePointDataExtractor:
    ...
    def _get_files_in_custom_list(
        ...
        :param site_domain: The site domain of sharepoint in Microsoft Graph.
        :param list_id: The list ID in Microsoft Graph.
        :param access_token: The access token for Microsoft Graph API authentication. If not provided, it will be fetched from self.
        :param file_formats: List of desired file formats.
        :return: A list of file details.
        :raises Exception: If there's an error in fetching file details.
        ...
        # Construct the URL based on whether a folder path is provided
        url = f"https://graph.microsoft.com/v1.0/sites/{site_domain}/lists/{list_id}/items"
        ...
        try:
            ...
            filtered_files = [obj for obj in files if obj['contentType']['name'] == "Document"]
            # print("Filtered Files:",filtered_files)
            return filtered_files
        except Exception as err:
            logger.error(f"Error in _get_files_in_custom_list: {err}")
            raise

    def get_group_members(
        self, group_id: str, access_token: Optional[str] = None
    ) -> List[Dict]:
        """
        Get the members of group

        :param group_id: The group ID in Microsoft Graph.
        :param access_token: The access token for Microsoft Graph API authentication. If not provided, it will be fetched from self.
        :return: A members of groups.
        ...
        try:
            ...
            logger.error(f"Error in get_group_members: {err}")
            raise
    ...
    def get_custom_list_file_permissions(
        self,
        site_domain: str,
        list_id: str,
        file_name: str,
        ...
        :param site_domain: The site domain in Microsoft Graph.
        :param list_id: The list ID of the file in Microsoft Graph.
        :param item_id: The item ID inside the list in Microsoft Graph.
        :return: A list of permission details.
        :raises Exception: If there's an error in fetching permission details.
        """
        ...
        url = f"https://graph.microsoft.com/v1.0/sites/{site_domain}/lists/{list_id}/items/{item_id}/driveitem/permissions"

        try:
            json_response = self._make_ms_graph_request(url, access_token)
            return json_response["value"]
        except Exception as err:
            logger.error(f"Error in get_custom_list_file_permissions: {err}")
            raise
    ...
    def get_read_access_entities(permissions):
        ...
        for permission in permissions:
            ...
            #     print("read_access_entities:",read_access_entities)
        return read_access_entities
    
    @staticmethod
    def extract_emails(data,group_data):
        """
        Extracts emails of the permission objects

        :param data: List of permission dictionaries.
        ...
        # emails = []
        for item in data:
            if "grantedToV2" in item:
                if "user" in item["grantedToV2"] and "email" in item["grantedToV2"]["user"]:
                    if item["grantedToV2"]["user"]["email"] not in unique_emails:  # Check if email is already present
                        unique_emails.add(item["grantedToV2"]["user"]["email"])
                    # emails.append(item["grantedToV2"]["user"]["email"])

                # elif "siteUser" in item["grantedToV2"] and "email" in item["grantedToV2"]["siteUser"]:
                #     emails.append(item["grantedToV2"]["siteUser"]["email"])
                # elif "group" in item["grantedToV2"] and "email" in item["grantedToV2"]["group"]:
                #     print('group permission',item["grantedToV2"]["group"]["id"])
                    
        for user in group_data:
            if user['mail'] not in unique_emails:  # Check if email is already present
    ...
    def get_custom_list_file_content_bytes(
        self,
        site_domain: str, 
        list_id: str, 
        file_name:str,
        ...
        :param list_id: The list ID in Microsoft Graph.
        :param file_name: The name of the file.
        :param item_id: The item ID in Microsoft Graph..
        :return: Bytes content of the file or None if there's an error.
        """
        access_token = self.access_token

        endpoint = f"https://graph.microsoft.com/v1.0/sites/{site_domain}/lists/{list_id}/items/{item_id}/driveitem/content"
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Failed to retrieve file content in get_custom_list_file_content_bytes. Status code: {response.status_code}, Response: {response.text}"
                )
                return None
            return response.content
        except requests.exceptions.RequestException as req_err:
    ...
    def _retrieve_custom_list_file_content(
        self,
        site_domain: str, 
        list_id: str, 
        file_name:str,
        ...
        if file_name.endswith(".docx"):
            return self.process_and_retrieve_custom_list_docx_content(
                site_domain, list_id, item_id, file_name
            )
        elif file_name.endswith(".pdf"):
            return self.process_and_retrieve_custom_list_pdf_content(
                site_domain, list_id, item_id, file_name
            )
        elif file_name.endswith(".csv"):
            return self.process_and_retrieve_custom_list_csv_content(
                site_domain, list_id, item_id, file_name
            )
        elif file_name.endswith(".xlsx"):
            return self.process_and_retrieve_custom_list_xlsx_content(
                site_domain, list_id, item_id, file_name
            )
        elif file_name.endswith(".pptx"):
            return self.process_and_retrieve_custom_list_pptx_content(
                site_domain, list_id, item_id, file_name
            )
    ...
    def process_and_retrieve_docx_content(
        ...
        try:
            document = extract_text_from_docx_bytes(file_bytes)
            return document
        except Exception as err:
            logger.error(f"Error processing document: {err}")
            return None
    ...
    def process_and_retrieve_pptx_content(
        ...
        """
        Process the content of a .pptx file and extract its text.

        :param site_id: The site ID in Microsoft Graph.
        :param drive_id: The drive ID in Microsoft Graph.
        :param folder_path: Path to the folder within the drive, can include subfolders.
        :param file_name: The name of the .pptx file.
        :param access_token: The access token for Microsoft Graph API authentication.
        :return: Text content of the .pptx file or None if there's an error.
        """
        file_bytes = self.get_file_content_bytes(
            site_id, drive_id, folder_path, file_name, access_token
        )
        ...
        try:
            ...
            # Extract text from all slides (limited success rate)
            text = ""
    ...
    def process_and_retrieve_csv_content(
        self,
        site_id: str,
        drive_id: str,
        folder_path: Optional[str],
        ...
        """
        Process the content of a .csv file and extract its text.
        ...
        :param access_token: The access token for Microsoft Graph API authentication.
        :return: Text content of the .csv file or None if there's an error.
        """
        file_bytes = self.get_file_content_bytes(
            site_id, drive_id, folder_path, file_name, access_token
        )
        ...
        if not file_name.endswith(".csv"):
            logger.error(f"File {file_name} is not a .csv file.")
            return None
    ...
    def process_and_retrieve_xlsx_content(
        self,
        site_id: str,
        drive_id: str,
        folder_path: Optional[str],
        ...
        """
        Process the content of a .xlsv file and extract its text.

        :param site_id: The site ID in Microsoft Graph.
        :param drive_id: The drive ID in Microsoft Graph.
        :param folder_path: Path to the folder within the drive, can include subfolders.
        :param file_name: The name of the .xlsv file.
        :param access_token: The access token for Microsoft Graph API authentication.
        :return: Text content of the .xlsv file or None if there's an error.
        """
        file_bytes = self.get_file_content_bytes(
            site_id, drive_id, folder_path, file_name, access_token
        )
        ...
        if not file_name.endswith(".xlsx"):
            logger.error(f"File {file_name} is not a .xlsv file.")
            return None

        try:
            xlsx_file = io.BytesIO(file_bytes)
            xlsx_data = xlsx_file.read().decode('latin-1')
            return xlsx_data
        except Exception as err:
            logger.error(f"Error processing document: {err}")
            return None
    ...
    def process_and_retrieve_pdf_content(
        ...
        """
        Process the content of a .pdf file and extract its text.
    ...
    def process_and_retrieve_custom_list_docx_content(
        self,
        site_domain: str,
        list_id: str,
        item_id: str,
        ...
        file_bytes = self.get_custom_list_file_content_bytes(
            site_domain, list_id, file_name, item_id
        )
        if file_bytes is None:
            return None
    ...
    # ) -> Optional[str]:
    #     """
    #     Process the content of a .docx file from a custom list and extract its text.
    ...
    def process_and_retrieve_custom_list_pptx_content(
        self,
        site_domain: str,
        list_id: str,
        item_id: str,
    ...
    def process_and_retrieve_custom_list_csv_content(
        self,
        site_domain: str,
        list_id: str,
        item_id: str,
    ...
    def process_and_retrieve_custom_list_xlsx_content(
        self,
        site_domain: str,
        list_id: str,
        item_id: str,
    ...
    def process_and_retrieve_custom_list_pdf_content(
        self,
        site_domain: str,
        list_id: str,
        item_id: str,
        ...
        :return: Text content of the .pdf file or None if there's an error.
        """
        file_bytes = self.get_custom_list_file_content_bytes(
            site_domain, list_id, file_name, item_id
        )
    ...
    def retrieve_custom_list_files_content(
        self,
        site_domain: str,
        site_name: str,
        list:List[Dict],
        ...
        """
        Retrieve contents of files from a custom document library in sharepoint, optionally filtering by file formats.
        :param site_domain: The domain of the site in Microsoft Graph.
        :param site_name: The name of the site in Microsoft Graph.
        :param list: custom list  sharepoint site (except wte,Shared Documents and Access Requests)
        :param file_formats: Optional; list of desired file formats to include.
        :return: Dictionary with file names as keys and a dictionary containing their content, location, and users_by_role as values.
        """
        ...
        files=self._get_files_in_custom_list(
            site_domain, list["id"], file_formats
        )
        ...
        return self._process_custom_list_files(site_domain,list["id"],files,file_formats)
    ...
    def _process_files(
        ...
        for file in files:
            ...
            if file_name and self._is_file_format_valid(file_name, file_formats):
                ...
                user_permissions = self.get_file_permissions(site_id, file["id"])

                groups_members=[]

                for group in user_permissions:
                    if "group" in group["grantedToV2"] and "email" in group["grantedToV2"]["group"]:
                        # print('group permission',group["grantedToV2"]["group"]["id"])
                        get_group_member=self.get_group_members(group["grantedToV2"]["group"]["id"])
                        
                        for group in get_group_member:
                            groups_members.append(group)
                

                users_by_role = self.extract_emails(user_permissions,groups_members)
    ...
    # def _process_custom_list_files(
    #     self,
    #     site_domain: str,
    #     list_id: str,
    #     files: List[Dict],
    ...
    #     :param site_domain: The site domain in Microsoft Graph.
    #     :param list_id: The drive ID in Microsoft Graph.
    #     :param files: List of files to process.
    #     :param file_formats: List of desired file formats.
    #     :return: A list of dictionaries, each mapping file names to their content and metadata.
    #     """
    ...
    #     for file in files:
    #         file_metadata1 = self._get_file_metadata(site_domain,list_id,item_id=file["id"])
    #         file_name=file_metadata1["name"]
    #         file_metadata=self._extract_file_metadata(file_metadata1)
            
    #         if file_name and self._is_file_format_valid(file_name, file_formats):

    #             content = self._retrieve_custom_list_file_content(
    #                 site_domain,list_id,file_name,item_id=file["id"]
    #             )
    #             user_permissions = self.get_custom_list_file_permissions(site_domain,list_id,file_name,item_id=file["id"])
    ...
    #             file_content = {
    #                 "content": content,
    #                 **self._format_metadata(file_metadata, file_name, users_by_role),
    #             }
    #             file_contents.append(file_content)
    ...
    def _process_custom_list_files(
        ...
        for file in files:
            ...
            if file_name and self._is_file_format_valid(file_name, file_formats):
                ...
                for group in user_permissions:
                    if "group" in group["grantedToV2"]:
                        
                        #get_group_member=self.get_group_members(group["grantedToV2"]["group"]["id"])
                        get_group_member=group["grantedToV2"]["group"]["id"]
                        # print("############################")
                        # print(get_group_member)
                        # for group in get_group_member:
                        #     groups_members.append(group)
                        groups_members.append(get_group_member)
                #users_by_role = self.extract_emails(user_permissions,groups_members)
                users_by_role =groups_members
    ...
    def _get_file_metadata(
        ...
        """
        Retrieve the metadata of file item.
        :param site_domain: SharePoint site domain.
        :param list_id: SharePoint list ID.
        :param item_id: SharePoint item ID.
        :return: metadata of the file item as a string
        """
        
        url = f"https://graph.microsoft.com/v1.0/sites/{site_domain}/lists/{list_id}/items/{item_id}/driveitem"
        ...
        try:
            ...
            logger.error(f"Error in _get_file_metadata: {err}")
            raise
    ...
    def _retrieve_file_content(
        ...
        if file_name.endswith(".docx"):
            ...
            return self.process_and_retrieve_csv_content(
                site_id, drive_id, folder_path, file_name
            )
        elif file_name.endswith(".xlsx"):
            return self.process_and_retrieve_xlsx_content(
                site_id, drive_id, folder_path, file_name
            )
        elif file_name.endswith(".pptx"):
